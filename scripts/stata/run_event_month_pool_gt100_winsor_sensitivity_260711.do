...
local C ev_ln_review_count ev_mean_rating ev_sd_rating ev_ln_mean_text_chars ln_lag_volumn_acc lag_avg_rating_acc lag_sd_acc ln_avg_com_RevPAR
estimates clear

* M0.1: change only the symmetric outcome/lag winsorization rule.
reghdfe lnRevenue_current_w199 ars_pool_ev `C' lnRevenue_lag_month_w199 if !missing(ars_pool_ev), absorb(hotel_id_num ym) vce(cluster hotel_id_num)
estimates store W199
reghdfe lnRevenue_current_w595 ars_pool_ev `C' lnRevenue_lag_month_w595 if !missing(ars_pool_ev), absorb(hotel_id_num ym) vce(cluster hotel_id_num)
estimates store W595
reghdfe lnRevenue_current_w195 ars_pool_ev `C' lnRevenue_lag_month_w195 if !missing(ars_pool_ev), absorb(hotel_id_num ym) vce(cluster hotel_id_num)
